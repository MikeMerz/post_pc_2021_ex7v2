...
class ExampleInstrumentedTest {
    ...
    @Test
    fun whenAddingCommentsTheyShouldRemainInEditScreen() {
        ...
        mainActivity.onActivity { it: MainActivity ->
            ...
            val editScreen = it.currentFocus
            ...
            assertNotEquals(editScreen,it.setContentView(R.layout.order_ready_screen))

        }
    }
    @Test
    fun whenEmptyUsernameShouldNotGoToEdit()
    {
        val mainActivity = ActivityScenario.launch<MainActivity>(MainActivity::class.java)
        mainActivity.moveToState(Lifecycle.State.CREATED)
        mainActivity.onActivity { it: MainActivity ->
            val userName = it.findViewById<EditText>(R.id.textView3)
            val firstScreen = it.currentFocus
            it.findViewById<Button>(R.id.buttonCur).performClick()
            assertEquals(it.currentFocus,firstScreen)
        }
    }
    @Test
